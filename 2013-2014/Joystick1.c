...
{
	short MAX_MOTOR_VAL = 70; // MAX percentage allowed
...
task main()
{
  ...
  while (true)
  {
    ...
    motor[conveyorMotor] = adjustValue(joystick.joy2_y1);

    if (joy1Btn(1)) // A-Button
    {
    	motor[motorA] = 100;
    }
    else if (joy1Btn(2)) // X-Button
    {
    	motor[motorA] = -100;
    }
    else
    ...
    if (joy2Btn(1)) // A-Button
    {
    	motor[motorB] = 100;
    }
    else if (joy2Btn(2)) // X-Button
    {
    	motor[motorB] = -100;
    }
    else
